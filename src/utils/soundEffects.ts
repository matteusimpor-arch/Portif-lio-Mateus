...
class SoundSystem {
  ...
  /**
   * Laser shot sound for Asteroid Defense & Arcade
   */
  public playLaser() {
    if (!this.enabled) return;
    try {
  ...
  /**
   * Pong / Brick paddle bounce sound
   */
  public playBounce(pitch: number = 440) {
    if (!this.enabled) return;
    try {
      this.initCtx();
      if (!this.ctx) return;
  ...
  /**
   * Explosion sound for Asteroids / Mines / Pinball
   */
