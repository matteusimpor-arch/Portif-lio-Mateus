...
export const TimeTravelSpiral: React.FC<TimeTravelSpiralProps> = ({ onComplete, onSkip }) => {
  ...
  useEffect(() => {
    ...
    // Microparticles vortex evolving dynamically
    ...
    const render = () => {
      ...
      // 1. Background color palette evolution
      // 2000-2014: Dark retro navy slate (#020617)
      // 2015-2019: Deep navy with subtle deep green reflections
      // 2020-2023: Deep black with forest green nuances
      // 2024-2026: Ultra deep black (#000402) with metallic green volumetric radiance
      if (calculatedYear < 2015) {
        ctx.fillStyle = 'rgba(2, 6, 23, 0.28)';
      } else if (calculatedYear < 2020) {
        ctx.fillStyle = 'rgba(2, 18, 14, 0.28)';
      } else if (calculatedYear < 2024) {
        ctx.fillStyle = 'rgba(1, 10, 6, 0.28)';
      } else {
        ctx.fillStyle = 'rgba(0, 4, 2, 0.26)';
      }
      ...
      if (spiralAlpha > 0.02) {
        for (let arm = 0; arm < spiralArms; arm++) {
          ...
          if (calculatedYear < 2015) {
            ...
            // 2015-2019: Introduction of deep green, teal, and luminous lines
            ctx.strokeStyle = arm % 2 === 0 ? `rgba(20, 184, 166, ${0.55 * spiralAlpha})` : `rgba(5, 150, 105, ${0.55 * spiralAlpha})`;
            ctx.lineWidth = 1.8;
          } else if (calculatedYear < 2024) {
            // 2020-2023: Metallic green, teal, technological reflections
            ctx.strokeStyle = arm % 2 === 0 ? `rgba(45, 212, 191, ${0.7 * spiralAlpha})` : `rgba(16, 185, 129, ${0.7 * spiralAlpha})`;
            ctx.lineWidth = 2.2;
          } else {
            // 2024-2026: Metallic Green structure + Emerald + Teal + Lime highlights
            ctx.strokeStyle = arm % 2 === 0 ? `rgba(45, 212, 191, ${0.85 * spiralAlpha})` : `rgba(163, 230, 53, ${0.85 * spiralAlpha})`;
            ctx.lineWidth = 2.8;
          }
      ...
      particles.forEach((p) => {
        ...
        if (calculatedYear < 2015) {
          ...
          // Teal / Deep Green / Cyan
          const transPalette = ['#14b8a6', '#0d9488', '#059669', '#38bdf8', '#2dd4bf'];
          p.color = transPalette[Math.floor(Math.random() * transPalette.length)];
        } else {
          // 2020-2026: Metallic Green, Emerald, Teal, Lime highlights
          const spacePalette = ['#2dd4bf', '#14b8a6', '#10b981', '#34d399', '#22c55e', '#a3e635'];
        ...
        if (p.isDigitalSquare && calculatedYear >= 2018) {
          ctx.fillRect(spiralX - p.size / 2, spiralY - p.size / 2, p.size, p.size);
        } else {
          ctx.beginPath();
          ctx.arc(spiralX, spiralY, p.size, 0, Math.PI * 2);
          ctx.fill();
        }
      });
      ...
      const coreGlow = ctx.createRadialGradient(centerX, centerY, 5, centerX, centerY, coreRadius);
      ...
      if (calculatedYear < 2015) {
        ...
        coreGlow.addColorStop(0.3, '#14b8a6');
        coreGlow.addColorStop(0.8, 'rgba(5, 150, 105, 0.3)');
      } else {
        coreGlow.addColorStop(0.3, '#2dd4bf');
        coreGlow.addColorStop(0.6, '#10b981');
        coreGlow.addColorStop(0.9, 'rgba(163, 230, 53, 0.25)');
      }
      ...
      if (calculatedYear === 2026) {
        ...
        ctx.strokeStyle = `rgba(45, 212, 191, ${Math.max(0, 0.6 - pulseR / 600)})`;
        ctx.lineWidth = 2;
        ctx.beginPath();
      ...
      if (rawProgress < 1) {
        ...
        // Thousands of particles disperse smoothly into the Space background
        setTimeout(() => {
          onComplete();
        }, 400);
      }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#000402] text-white flex flex-col justify-between overflow-hidden select-none">
      <canvas ref={canvasRef} className="absolute inset-0 w-full h-full pointer-events-none" />

      {/* Top Minimalist Header */}
      <div className="relative z-10 p-3.5 sm:p-5 flex items-center justify-between backdrop-blur-md bg-black/40 border-b border-teal-500/20 font-mono">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-lg bg-teal-950/80 border border-teal-500/40 text-teal-300">
            <Compass className="w-5 h-5 animate-spin" />
          </div>
          <div>
            <div className="text-xs sm:text-sm font-bold text-teal-300 flex items-center gap-2">
              <span>SALTO TEMPORAL</span>
              <span className="text-[10px] px-2 py-0.5 rounded bg-teal-950 text-lime-300 border border-teal-700">
        ...
        <button
          onClick={handleSkip}
          className="px-3 py-1.5 rounded bg-black/60 hover:bg-teal-950 border border-teal-900 hover:border-teal-500 text-teal-300 text-xs font-mono font-bold flex items-center gap-1.5 cursor-pointer shadow-lg active:scale-95 transition"
        >
          <FastForward className="w-3.5 h-3.5 text-lime-400" />
          <span>Avançar</span>
        </button>
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center pointer-events-none my-auto space-y-3">
        <div className="text-8xl sm:text-9xl md:text-[150px] font-black font-vt323 tracking-wider text-transparent bg-clip-text bg-gradient-to-b from-white via-teal-300 to-lime-400 drop-shadow-[0_0_50px_rgba(20,184,166,0.9)] animate-pulse">
          {currentYear}
        </div>

        <p className="text-xs sm:text-sm font-mono text-lime-300 font-bold max-w-md text-center bg-black/80 px-4 py-2 rounded-xl border border-teal-500/30 backdrop-blur-md">
          {currentYear < 2015
            ...
            : 'MATEUS SPACE 2026 • O FUTURO OPERACIONAL'}
        </p>
      </div>

      {/* Bottom Progress Bar */}
      <div className="relative z-10 p-3.5 sm:p-5 backdrop-blur-md bg-black/40 border-t border-teal-500/20 font-mono space-y-2">
        <div className="w-full bg-black/80 h-2.5 rounded-full border border-teal-900 overflow-hidden p-0.5">
          <div
            className="h-full bg-gradient-to-r from-cyan-500 via-teal-400 to-lime-400 rounded-full transition-all duration-75 shadow-[0_0_12px_rgba(45,212,191,0.6)]"
